...
import { userCodeEditorStore } from "@/store/useCodeEditorStore";
...
const LanguageSelector = ({hasAccess} : {hasAccess: boolean}) => {
  ...
  const {language, setLanguage} = userCodeEditorStore()
  const mounted = useMounted()

  const currentLanguageObj = LANGUAGE_CONFIG[language]


  // this will close the dropdown when the user clicks outside
  useEffect(() => {
  ...
  return (
    <div className="relative " ref={dropdownRef}>
      ...
      className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border 
         border-gray-800/50 hover:border-gray-700 ${!hasAccess && language !== "javascript" ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        {/* Decoration */}
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          className="absolute top-full left-0 mt-2 w-64 rounded-lg bg-[#1e1e2e]/80 border border-gray-800/50 shadow-lg backdrop-blur-xl  ring-1 ring-black/5 py-2 z-50" 
          >
            <div className="px-3 pb-2 mb-2 border-b border-gray-800/50">
              <p className="text-xs text-gray-400 font-medium">Select a language</p>
            </div>
